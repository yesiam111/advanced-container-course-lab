...
Hệ thống giám sát báo: image `smartapp:1.0` trên registry **không còn khớp chữ ký** đã ký tuần trước. Cùng lúc, đợt rà soát dependency đang trễ hạn. Một image "sạch CVE" vẫn có thể bị tráo nguồn gốc — cần phải chứng minh điều đó, tìm thành phần nguy hiểm, và dựng hàng rào để không bị tái diễn.
...
1. **Chứng minh image bị tráo.** Dùng chữ ký để cho thấy bản đã ký tuần trước là hợp lệ, còn image đang nằm ở tag `:1.0` **không qua được xác minh**. Ghi `SWAPPED_TAG` vào `answers.env`.
2. **Tìm thành phần nguy hiểm.** Tạo **SBOM** của image đáng ngờ, tìm thư viện lỗ hổng được cài lén. Ghi `VULN_LIB` vào `answers.env`.
3. **Dựng pipeline gác cổng.** Viết `pipeline.sh <image>` — quét CVE và xác minh chữ ký — **exit code != 0** nếu có CVE mức CRITICAL hoặc chữ ký sai, **exit code = 0** nếu image sạch và hợp lệ.
...
Giữ lại image đã sửa. Ở Lab 04, một báo cáo pentest cho thấy container smartapp **đọc được `/etc/shadow` của host** — cần tái hiện lỗ hổng rồi hardening để chặn.
